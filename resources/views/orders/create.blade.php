...
<style>
    ...
    .type-btn{background:rgba(23,31,51,.5);border:1.5px solid rgba(173,198,255,.1);border-radius:12px;padding:14px 8px;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;transition:all .2s;border:1.5px solid transparent}
        ...
        <div class="glass-card rounded-xl p-6">
            ...
            <div class="max-h-80 overflow-y-auto pr-1" id="svc-list">
                <div class="text-center py-8 text-outline" id="svc-loading" style="display:none">
                    <span class="material-symbols-outlined animate-spin text-[28px] block mb-2">progress_activity</span>Loading services...
                </div>
        ...
        <div class="glass-card rounded-xl p-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="url" id="order-link" class="w-full glass-input py-2.5 px-3 text-sm" placeholder="https://...">
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <input type="number" id="qty-num" class="w-24 glass-input py-1.5 px-3 text-sm text-center" oninput="syncSlider()">
                    </div>
                    <input type="range" id="qty-range" min="100" max="10000" step="1" value="1000"
                        class="w-full accent-blue-400" oninput="syncNum()">
                ...
                <div class="bg-surface-container-low rounded-xl p-4 border border-outline-variant/30" style="background: rgba(0,0,0,0.2)">
...
<script>
    ...
    function buildTypeGrid(platform, platformKws) {
        ...
        const searchTerms = [platform.toLowerCase(), ...platformKws.map(k => k.toLowerCase())];
        ...
        // Fallback: If no matches, use all categories to prevent empty screen
        const catsToUse = matchingCats.length > 0 ? matchingCats : ALL_CATS;

        const availableTypes = SERVICE_TYPES.filter(t => {
            if (t.label === 'Everything') return true;
            return catsToUse.some(c =>
    ...
    function selectType(type, el, catsToUse) {
        ...
        let targetCats = (type.label === 'Everything') 
            ? catsToUse 
            : catsToUse.filter(c => type.keywords.some(kw => c.name.toLowerCase().includes(kw)));

        if(targetCats.length === 0) targetCats = catsToUse; // Final safety fallback

        loadServicesForCats(targetCats);
        goStep(3);
    }
