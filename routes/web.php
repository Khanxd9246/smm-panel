...
Route::middleware(['auth', 'admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    // ── Pricing helpers (used by AI pricing page) ─────────────────────────
    Route::patch('categories/{category}', function (\Illuminate\Http\Request $request, \App\Models\Category $category) {
        $category->update(['profit_margin' => $request->profit_margin]);
        return response()->json(['success' => true]);
    })->name('categories.update');

    Route::patch('providers-margin/{provider}', function (\Illuminate\Http\Request $request, \App\Models\ApiProvider $provider) {
        $provider->update(['profit_margin' => $request->profit_margin]);
        return response()->json(['success' => true]);
    })->name('providers.margin');
});
